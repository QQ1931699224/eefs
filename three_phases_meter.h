...
u8 meter_getNoPowerFlag(u8* data);	                                                    //掉电标志,5字节，不带crc，存1份 get set
u8 meter_setNoPowerFalg(u8* data,u16 len);
u8 meter_getSendParameter(u8* data);                                                    //发送参数，24字节，不带crc，存1份 get set
u8 meter_setSendParameter(u8* data, u16 len);
u8 meter_getUpgrade(u8* data);                                                          //升级参数，415字节，不带crc，存1份 get set
u8 meter_setUpgrade(u8* data, u16 len);
u8 meter_getFirstJudge(u8* data);                                                       //首次判断，4字节，不带crc，存1份 get set
u8 meter_setFirstJudge(u8* data, u16 len);
u8 meter_getEnergy(u8* data);                                                           //电能，160字节带crc，存4份 get set
u8 meter_setEnergy(u8* data, u16 len);
u8 meter_getCheckMeter(u8* data);                                                       //校表参数 78字节 1份 不带crc get set
